...
- [상단 알림 팝오버 바깥 클릭 닫기 구현](#상단-알림-팝오버-바깥-클릭-닫기-구현)
...
# 상단 알림 팝오버 바깥 클릭 닫기 구현

> task4에 구현사항들 추가

## 답변

상단 알림 팝오버가 열린 상태에서 메인 화면 빈 영역을 클릭하면 알림창이 닫히도록 구현했다.
...
- 알림 팝오버의 backdrop과 팝오버를 `createPortal(..., document.body)`로 렌더링하도록 변경함.
- 투명 backdrop을 `fixed inset-0`로 화면 전체에 깔도록 변경함.
- backdrop 클릭 시 `setIsNotificationOpen(false)`가 호출되도록 함.
- 알림 팝오버 내부 클릭은 `stopPropagation`으로 닫히지 않게 처리함.
- 화면은 어둡게 하지 않고 `bg-transparent`를 유지함.
- 기존 알림 탭과 알림 목록 UI는 유지함.
